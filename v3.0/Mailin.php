...
//        if (!is_string($data) || !strlen($data)) {
    ...
    */
    ...
    /*
        ...
        @options data {Integer} limit: Maximum number of records per request is 50, if there are more than 50 processes then you can use this parameter to get next 50 results [Mandatory]
        @options data {Integer} offset: This should be a valid number between 0-50 [Mandatory]
    */
    public function get_lists($data)
    {
     ...
     /*
        Create a new list.
        @param {Array} data contains php array with key value pair.
        @options data {String} name: Desired name of the list to be created [Mandatory]
    ...
    */
    ...
    /*
        ...
        @param {Array} data contains php array with key value pair.
        @options data {Integer} listId: Id of list to get details [Mandatory]
    */
    public function get_list($data)
    {
        return $this->get("contacts/lists/" . $data['listId'], "");
    }
    ...
    /*
        Update a list.
        @param {Array} data contains php array with key value pair.
        @options data {Integer} listID: Id of list to be modified [Mandatory]
        @options data {String} name: Desired name of the list to be modified [Optional]
        @options data {Integer} folderId: Folder ID [Mandatory]
        warning: Only one parameter can be update at once
    */
    public function update_list($data)
    {
        $id = $data['listId'];
        unset($data['listId']);
        return $this->put("contacts/lists/" . $id, json_encode($data));
    }
    /*
        Delete a specific list.
        @param {Array} data contains php array with key value pair.
        @options data {Integer} listId: Id of list to be deleted [Mandatory]
    */
    public function delete_list($data)
    {
        $id = $data['listId'];
        unset($data['listId']);
        return $this->delete("contacts/lists/" . $id, "");
    }

    /*
        Get contacts for the given lists.
        @param {Array} data contains php array with key value pair.
        @options data {Array} listid: These are the list ids to get their data. The ids found will display records [Mandatory]
        @options data {Datetime} modifiedSince: Filter (urlencoded) the contacts modified after a given UTC date-time (YYYY-MM-DDTHH:mm:ss.SSSZ). Prefer to pass your timezone in date-time format for accurate result. [Optional]
        @options data {Integer} limit: Number of documents per page [Optional]
        @options data {Integer} offset: Index of the first document of the page [Optional]
    */
    // TODO: something wrong with datetime object
    public function get_list_contacts($data)
    {
        $id = $data['listId'];
        unset($data['listId']);
        return $this->post("contacts/lists/" . $id . "/contacts", json_encode($data));
    }

     /*
        Add already existing contact in the SendinBlue contacts to the list.
        @param {Array} data contains php array with key value pair.
        @options data {Integer} listId: Id of list to link users in it [Mandatory]
        @options data {Array} emails: Email address of the already existing user(s) in the SendinBlue contacts. Example: "test@example.net". You can use commas to separate multiple users [Mandatory]
    */

    public function add_contacts_list($data)
    {
        $id = $data['listId'];
        unset($data['listId']);
        return $this->post("contacts/lists/" . $id . "/contacts/add", json_encode($data));
    }
    ...
    */
    public function delete_contacts_list($data)
    {
        $id = $data['listId'];
        unset($data['listId']);
        return $this->post("contacts/lists/" . $id . "/contacts/remove", json_encode($data));
    }
    ...
    /*
        ...
        @param {Array} data contains php array with key value pair.
        @options data {Integer} limit: Number of documents per page [optional]
        @options data {Integer} offset: Index of the first document of the page
        @options data {Datetime} modifiedSince: Filter (urlencoded) the contacts modified after a given UTC date-time (YYYY-MM-DDTHH:mm:ss.SSSZ). Prefer to pass your timezone in date-time format for accurate result. [Optional]
    */
    public function get_all_contacts($data)
    {
        return $this->get("contacts", json_encode($data));
    }

    /*
        Create a contact.
        @options data {string} email: Email address of the user. Mandatory if attributes.sms is not passed
        @options data {object} attributes: Values of the attributes to fill. The attributes must exist in you contact database
        @options data {Boolean} emailBlacklisted: Blacklist the contact for emails (emailBlacklisted = true)
        @options data {array of integers} smsBlacklisted: Blacklist the contact for SMS (smsBlacklisted = true)
        @options data {Integer} listIds: Ids of the lists to add the contact to
        @options data {Boolean} updateEnabled: Facilitate to update existing contact in same request (updateEnabled = true)
        @options data {array of strings} smtpBlacklistSender: SMTP forbidden sender for contact. Use only for email Contact ( only available if updateEnabled = true )
    */
    ...
    /*
        ...
        @param {Array} data contains php array with key value pair.
    */
    public function retrieve_contact_information($data)
    {
        $email = $data['email'];
        unset($data['email']);
        return $this->get("contacts/" . $email, json_encode($data));
    }

    /*
        Updates a contact.
        @param {Array} data contains php array with key value pair.
        @options data {object} attributes:
        @options data {boolean} emailBlacklisted: Blacklist the contact for emails (emailBlacklisted = true)
        @options data {boolean} smsBlacklisted: Blacklist the contact for SMS (smsBlacklisted = true)
        @options data {array of integers} listIds: Blacklist the contact for SMS (smsBlacklisted = true)
        @options data {array of integers} unlinkListIds: Ids of the lists to remove the contact from
        @options data {array of strings} smtpBlacklistSender: SMTP forbidden sender for contact. Use only for email Contact
    */
    public function update_contact($data)
    {
        $email = $data['email'];
        unset($data['email']);
        return $this->put("contacts/" . $email, json_encode($data));
    }
    ...
    */
    public function delete_contact($data)
    {
        $email = $data['email'];
        unset($data['email']);
        return $this->delete("contacts/" . $email, json_encode($data));
    }
    ...
    /*
        ...
        @param {Array} data contains php array with key value pair.
    */
    public function get_campaigns_statistics_of_contact($data)
    {
        $email = $data['email'];
        unset($data['email']);
        return $this->get("contacts/" . $email . "/campaignStats", json_encode($data));
    }

    /*
        Get all folders detail.
        @param {Array} data contains php array with key value pair.
        @options data {Integer} limit: Maximum number of records per request is 50, if there are more than 50 folders then you can use this parameter to get next 50 results [Mandatory]
        @options data {Integer} offset: This should be a valid number between 0-50 [Mandatory]
    */
    public function get_folders($data)
    {
        return $this->get("contacts/folders",json_encode($data));
    }
    ...
    */
    ...
    {
        return $this->post("contacts/folders", json_encode($data));
    }

    /*
        Get a particular folder detail.
        @param {Array} data contains php array with key value pair.
        @options data {Integer} folderId: Id of folder to get details [Mandatory]
    */
    public function get_folder($data)
    {
        return $this->get("contacts/folders/".$data['folderId'], "");
    }

    /*
        Update an existing folder.
        @param {Array} data contains php array with key value pair.
        @options data {Integer} folderId: Id of folder to be modified [Mandatory]
    ...
    */
    public function update_folder($data)
    {
        $id = $data['folderId'];
        unset($data['folderId']);
        return $this->put("contacts/folders/" . $id, json_encode($data));
    }

    /*
        Delete a specific folder information.
        @param {Array} data contains php array with key value pair.
        @options data {Integer} folderId: Id of folder to be deleted [Mandatory]
    */
    public function delete_folder($data)
    {
        return $this->delete("contacts/folders/".$data['folderId'], "");
    }
    ...
    /*
        ...
        @options data {Integer} folderId: Id of folder to be modified [Mandatory]
        @options data {Integer} limit: Maximum number of records per request is 50, if there are more than 50 processes then you can use this parameter to get next 50 results [Mandatory]
        @options data {Integer} offset: This should be a valid number between o-50 [Mandatory]
    */
    public function get_folder_lists($data)
    {
        $id = $data['folderId'];
        unset($data['folderId']);
        return $this->get("contacts/folders/" . $id. "/lists", json_encode($data));
    }
